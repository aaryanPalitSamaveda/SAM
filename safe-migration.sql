-- ============================================
-- Safe Database Migration for New Supabase Project
...
-- Drop triggers first
DROP TRIGGER IF EXISTS on_email_reply_received ON public.email_replies;
DROP TRIGGER IF EXISTS on_campaign_created ON public.campaigns;
DROP TRIGGER IF EXISTS on_draft_approved ON public.email_drafts;
DROP TRIGGER IF EXISTS on_draft_created ON public.email_drafts;
DROP TRIGGER IF EXISTS on_email_sent ON public.sent_emails;

-- Drop functions
DROP FUNCTION IF EXISTS public.handle_email_reply();
DROP FUNCTION IF EXISTS public.create_campaign_settings();
DROP FUNCTION IF EXISTS public.update_draft_analytics();
...
DROP TABLE IF EXISTS public.campaign_settings CASCADE;
...
DROP TABLE IF EXISTS public.sender_accounts CASCADE;
...
CREATE TABLE public.contacts (
  ...
  enriched_data JSONB,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  CONSTRAINT fk_contacts_batch FOREIGN KEY (upload_batch_id) REFERENCES public.upload_batches(id) ON DELETE CASCADE
);
...
-- Add campaign tracking columns to sent_emails (if not already added)
DO $$ 
BEGIN
  IF NOT EXISTS (
...
DROP POLICY IF EXISTS "Allow all on contact_lists" ON public.contact_lists;
DROP POLICY IF EXISTS "Allow all on contact_list_members" ON public.contact_list_members;
DROP POLICY IF EXISTS "Allow all on campaigns" ON public.campaigns;
DROP POLICY IF EXISTS "Allow all on campaign_sequences" ON public.campaign_sequences;
DROP POLICY IF EXISTS "Allow all on campaign_settings" ON public.campaign_settings;
DROP POLICY IF EXISTS "Allow all on campaign_contacts" ON public.campaign_contacts;
DROP POLICY IF EXISTS "Allow all on email_replies" ON public.email_replies;
